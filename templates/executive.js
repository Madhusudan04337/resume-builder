(function() {
  ...
  window.templates.executive = function(S, SOCIAL_DEFS) {
    ...
    if (P.address) sidebar += '<div style="margin-bottom:14px">📍 ' + esc(P.address) + '</div>';
    
    SOCIAL_DEFS.slice(2).forEach(function(sd){
      if (S.socialEnabled[sd.key] && P[sd.key]) sidebar += '<div style="margin-bottom:6px;display:flex;align-items:center;gap:4px;word-break:break-all"><b>' + sd.icon + '</b> ' + esc(P[sd.key]) + '</div>';
    });
       ...
       if (secName === "Experience" && X.length) {
         ...
         X.forEach(function(e){ 
           main += '<div style="margin-bottom:12px">' +
             '<div style="display:flex;justify-content:space-between;font-weight:700;color:#1e293b;font-size:11.5px"><span>' + esc(e.role) + '</span><span>' + esc(dr(e.start, e.end)) + '</span></div>' +
         ...
         Pr.forEach(function(p){ 
           main += '<div style="margin-bottom:10px">' +
             '<div style="font-weight:700;font-size:11.5px;color:#1e293b">' + esc(p.name) + (p.tech ? ' <span style="font-weight:500;color:#475569;font-size:10.5px">| ' + esc(p.tech) + '</span>' : '') + '</div>' + 
             buls(p.bullets) + 
           '</div>'; 
         });
         ...
         E.forEach(function(e){ 
           main += '<div style="margin-bottom:10px">' +
             '<div style="display:flex;justify-content:space-between;font-weight:700;color:#1e293b;font-size:11.5px"><span>' + esc(e.university) + '</span><span>' + esc(dr(e.start, e.end)) + '</span></div>' +
         ...
         Ct.forEach(function (c) {
           var np = '<span class="rb" style="color:#1e293b;font-weight:700;font-size:11px">' + esc(c.name) + '</span>';
         ...
         L.forEach(function (e) {
           main += '<div style="margin-bottom:12px">' +
             '<div style="display:flex;justify-content:space-between;font-weight:700;color:#1e293b;font-size:11.5px"><span>' + esc(e.org) + '</span><span>' + esc(dr(e.start, e.end)) + '</span></div>' +
